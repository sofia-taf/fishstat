merge_capture <- function()
{
  x <- capture

  # links
  area_link <- match(capture$area, area$area)
  country_link <- match(capture$country, country$country)
  measure_link <- match(capture$measure, measure$measure)
  source_link <- match(capture$source, source$source)
  species_link <- match(capture$species, species$species)
  status_link <- match(capture$status, status$status)
