merge_production <- function()
{
  x <- production

  # links
  area_link <- match(production$area, area$area)
  country_link <- match(production$country, country$country)
  measure_link <- match(production$measure, measure$measure)
  source_link <- match(production$source, source$source)
  species_link <- match(production$species, species$species)
  status_link <- match(production$status, status$status)
  ...
  # source
  x$source_name <- source$source_name[area_link]
